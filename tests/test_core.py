...
from coderover import Agent, LLM, Config, ALL_TOOLS, __version__
from coderover.context import ContextManager, estimate_tokens
from coderover.session import save_session, load_session, list_sessions
from coderover.tools import get_tool
...
def test_session_save_load():
    ...
    # cleanup
    pathlib.Path.home().joinpath(".coderover/sessions/pytest_test_session.json").unlink()
...
def test_session_name_is_sanitized():
    ...
    path = pathlib.Path.home().joinpath(".coderover/sessions/Research-Notes.json")
    assert path.exists()
...
def test_edit_tracks_changed_files(tmp_path):
    from coderover.tools.edit import _changed_files
